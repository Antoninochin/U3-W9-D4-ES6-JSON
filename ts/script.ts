...
class Abbigliamenti {
    ...
    constructor(id: number, codprod: number, collezione: string, modello: number, quantità: number, disponibile: string,){
        this.id = id;
...
class Capo extends Abbigliamenti{
    ...
    constructor(id: number,codprod: number,collezione: string,modello: number,quantità: number,disponibile: string,capo: string,color: string, przIvaEscl: number,przIvaIncl: number,saldo: number){
...
for(let i =0; i < abb.length;i++){
    ...
    document.querySelector('.card') as HTMLElement += `< div class="col">${indumento.id} ${indumento.codprod} ${indumento.collezione} ${indumento.modello} ${indumento.quantità} ${indumento.disponibile} ${indumento.capo} ${indumento.color} ${indumento.przIvaEscl} ${indumento.przIvaIncl} ${indumento.saldo}</div>`;
}
